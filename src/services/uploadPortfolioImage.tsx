...
export async function selectAndUploadPortfolioImage() {
  try {
    ...
    const storageRef = ref(storage, `portfolioImages/${user.uid}`);
    ...
    if (userTypeDoc.exists()) {
      ...
      if (!data.portfolioImageUrl) {
        // Se o campo 'profileImageUrl' não existir, cria o campo e atualiza
        await updateDoc(userTypeDocRef, { portfolioImageUrl: downloadURL });
        console.log("Campo 'portfolioImageUrl' criado e foto de perfil atualizada com sucesso!");
      } else {
      ...
      await setDoc(userTypeDocRef, { portfolioImageUrl: downloadURL });
